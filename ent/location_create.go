...
func (lc *LocationCreate) SetDept(f float64) *LocationCreate {
	lc.mutation.SetDept(f)
	return lc
}
...
func (lc *LocationCreate) AddEarthquakeIDs(ids ...int) *LocationCreate {
	lc.mutation.AddEarthquakeIDs(ids...)
	return lc
}

// AddEarthquakes adds the "earthquakes" edges to the Earthquake entity.
func (lc *LocationCreate) AddEarthquakes(e ...*Earthquake) *LocationCreate {
	ids := make([]int, len(e))
	for i := range e {
		ids[i] = e[i].ID
	}
	return lc.AddEarthquakeIDs(ids...)
}
...
func (lc *LocationCreate) check() error {
	...
	if _, ok := lc.mutation.Dept(); !ok {
		return &ValidationError{Name: "dept", err: errors.New(`ent: missing required field "Location.dept"`)}
	}
	return nil
}
...
func (lc *LocationCreate) createSpec() (*Location, *sqlgraph.CreateSpec) {
	...
	if value, ok := lc.mutation.Dept(); ok {
	...
	if nodes := lc.mutation.EarthquakesIDs(); len(nodes) > 0 {
