...
func (tx *Tx) init() {
	...
	tx.Magnitude = NewMagnitudeClient(tx.config)
	tx.Request = NewRequestClient(tx.config)
	tx.Role = NewRoleClient(tx.config)
	tx.SchemaMigration = NewSchemaMigrationClient(tx.config)
	tx.Time = NewTimeClient(tx.config)
	tx.Type_eathquake = NewTypeEathquakeClient(tx.config)
...
// applies a query, for example: AssociatedEvent.QueryXXX(), the query will be executed
