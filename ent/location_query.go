...
func (lq *LocationQuery) QueryEarthquakes() *EarthquakeQuery {
	query := (&EarthquakeClient{config: lq.config}).Query()
...
func (lq *LocationQuery) Clone() *LocationQuery {
	...
	return &LocationQuery{
		...
		withEarthquakes: lq.withEarthquakes.Clone(),
...
func (lq *LocationQuery) WithEarthquakes(opts ...func(*EarthquakeQuery)) *LocationQuery {
	query := (&EarthquakeClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withEarthquakes = query
	return lq
}
...
//		GroupBy(location.FieldLongitude).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Location.Query().
//		Select(location.FieldLongitude).
//		Scan(ctx, &v)
...
func (lq *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	if query := lq.withEarthquakes; query != nil {
		if err := lq.loadEarthquakes(ctx, query, nodes,
			func(n *Location) { n.Edges.Earthquakes = []*Earthquake{} },
			func(n *Location, e *Earthquake) { n.Edges.Earthquakes = append(n.Edges.Earthquakes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LocationQuery) loadEarthquakes(ctx context.Context, query *EarthquakeQuery, nodes []*Location, init func(*Location), assign func(*Location, *Earthquake)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(earthquake.FieldLocationID)
	}
	query.Where(predicate.Earthquake(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.EarthquakesColumn), fks...))
	}))
